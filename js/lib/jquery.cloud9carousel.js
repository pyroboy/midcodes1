...
;(function($) {
  ...
  var Item = function( element, options ) {
	...
	// $(element).css('webkitFilter',filterVal);
	 
	 
	   //$(element).css('filter',filterVal);
	   //$(element).css('mozFilter',filterVal);
	   //$(element).css('oFilter',filterVal);
	  // $(element).css('msFilter',filterVal);
    if( options.mirror && this.element.tagName === 'IMG' ) {
      ...
      $reflection.css( 'margin-top', options.mirror.gap + 'px' );
	   $reflection.css( 'max-width', '200px' );
  ...
  $.fn.Cloud9Carousel = function( options ) {
    return this.each( function() {
      ...
      options = $.extend( {
        ...
        fps: 50,              // fixed frames per second (if smooth animation is off)
